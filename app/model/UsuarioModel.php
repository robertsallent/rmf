<?php
/*
 FICHERO: app/controllers/UsuarioModel.php
 CLASE: UsuarioModel
...
class UsuarioModel{
	...
	// Retorna un array de UsuarioModel
	public static function get():array{
	    $consulta = "SELECT * FROM usuarios";
	    return DBPS::selectAll($consulta, 'UsuarioModel');
	}
	
	// Este método recupera un usuario de la BDD a partir del nombre de usuario
	// Retorna NULL si el usuario no existe
	public static function getUsuario($u):?UsuarioModel{
	    $consulta = "SELECT * FROM usuarios WHERE user=?;";
	    return DBPS::select($consulta, 'UsuarioModel', [$u]);
	}
	
	// Este método recupera un usuario de la BDD a partir del identificador único
	// Retorna NULL si el usuario no existe
	public static function getById($id=0):?UsuarioModel{
	    $consulta = "SELECT * FROM usuarios WHERE id=?;";
	    return DBPS::select($consulta,'UsuarioModel',[$id]);
	}
